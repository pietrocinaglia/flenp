...
// "colorize" the widget name
    $.widget("flowchart.flowchart", {
        ...
        options: {
            ...
            data: {},
            distanceFromArrow: 3,
            blockOperatorClass: 'flowchart-block-operator',
            stepOperatorClass: 'flowchart-step-operator',
        ...
        _initEvents: function () {
            ...
            this.objs.layers.operators.on('click', '.flowchart-operator', function (e) {
                ...
                if ($(e.target).closest('.flowchart-operator-delete').length == 1) {
                    self._deleteOperator($(this).data('operator_id'), false);
                }else if ($(e.target).closest('.flowchart-operator-connector').length == 0) {
        ...
        getOperatorCompleteData: function (operatorData) {
            ...
            if (typeof infos.type == 'undefined') {
                infos.class = this.options.stepOperatorClass;
            } else if (infos.type == 'step') {
                infos.class = this.options.stepOperatorClass;
            } else if (infos.type == 'block') {
                infos.class = this.options.blockOperatorClass;
            }
        ...
        _getOperatorFullElement: function (operatorData) {
            ...
            var $operator = $('<div class="flowchart-operator" title="' + (infos.description == 'undefined' ? '' : infos.description) + '"></div>');
            ...
            this.delete_button = $( "<button>", {
                text: "X",
                "class": "flowchart-operator-delete"
            })
            .appendTo( $operator )
            .button();
            ...
            if (infos.cmd != 'undefined') {
                var $operator_description = $('<div class="flowchart-operator-description"></div>');
                $operator_description.html(infos.cmd);
                $operator_description.appendTo($operator);
            } else if (infos.description != 'undefined') {
                var $operator_description = $('<div class="flowchart-operator-description"></div>');
                $operator_description.html(infos.description);
                $operator_description.appendTo($operator);
            }
        ...
        _createSubConnector: function (connectorKey, connectorInfos, fullElement) {
            ...
            if (connectorInfos.hasOwnProperty("label")){
        ...
        createOperator: function (operatorId, operatorData) {
            ...
            if (this.options.canUserMoveOperators) {
                ...
                fullElement.operator.draggable({
                    ...
                    handle: '.flowchart-operator-title, .flowchart-operator-description, .flowchart-block-operator, .flowchart-step-operator, .flowchart-operator-delete',
                    start: function (e, ui) {
        ...
        _connecterMouseOver: function (linkId) {
            if (this.selectedLinkId != linkId) {
                this.colorizeLink(linkId, this.options.defaultSelectedLinkColor); //aaa
            }
        },
        ...
        setOperatorDescription: function (operatorId, description) {
            this.data.operators[operatorId].internal.els.description.html(description);
            if (typeof this.data.operators[operatorId].properties == 'undefined') {
                this.data.operators[operatorId].properties = {};
            }
            this.data.operators[operatorId].properties.description = description;
            this._refreshInternalProperties(this.data.operators[operatorId]);
            this.callbackEvent('afterChange', ['operator_description_change']);
        },
        ...
        getOperatorDescription: function (operatorId) {
            return this.data.operators[operatorId].internal.properties.description;
        },
